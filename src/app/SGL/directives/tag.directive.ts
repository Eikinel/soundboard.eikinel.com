...
export class TagDirective implements OnInit, AfterViewInit, OnDestroy {
        ...
        if (tagContents?.length) {
          ...
          setTimeout(() => {
            this.getTagsInputContent().forEach((name: string) => {
              ...
              if (
                !this.tagsFormArray.value.filter(
  ...
  public ngOnInit(): void {
    ...
    hostClasses.forEach((hostClass: string) =>
      this.element.classList.add(hostClass),
    );

    fromEvent(this.element, "keydown")
      .pipe(takeUntil(this._destroyed))
      .subscribe((e: KeyboardEvent) =>
        this._listeners[e.key] ? this._listeners[e.key](e) : 0,
      );
  ...
  private toTagComponent(tag: Tag | Partial<Tag>): void {
    ...
    Object.keys(styles).forEach(
      (property: string) => (element.style[property] = styles[property]),
    );
  ...
  private fromTagComponent(): string {
    const lastComponentRef: ComponentRef<TagComponent> =
      this._componentRefs[this._componentRefs.length - 1];
